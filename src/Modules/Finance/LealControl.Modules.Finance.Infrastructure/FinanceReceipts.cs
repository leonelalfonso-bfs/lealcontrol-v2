...
public static class FinanceReceipts
{
    public static IEndpointRouteBuilder MapFinanceReceiptEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapPost("", async (CreateCollectionReceiptRequest body, FinanceDbContext db, LealControl.BuildingBlocks.Tenancy.ITenantContext tenant, CancellationToken ct) =>
        {
            ...
            if (movement is null && cheque is null) db.Movements.Add(new FinancialMovement { Id = Guid.NewGuid(), TenantId = tenantId, AccountId = body.AccountId, Kind = FinancialMovementKind.Credit, Amount = body.Amount, Currency = currency, OperationDateUtc = body.ReceiptDateUtc, Description = body.Description.Trim(), ExternalReference = number, LinkedEntityType = "CollectionReceipt", LinkedEntityId = receiptId, ReconciliationStatus = FinancialReconciliationStatus.Reconciled, CreatedAtUtc = DateTime.UtcNow });
        ...
        detail.MapGet("/accounts/{accountId:guid}/movements-detail", async (Guid accountId, FinanceDbContext db, LealControl.BuildingBlocks.Tenancy.ITenantContext tenant, CancellationToken ct) =>
        {
            ...
            var running = account.OpeningBalance; var result = rows.Select(x => { running += x.Kind == FinancialMovementKind.Credit ? x.Amount : -x.Amount; return new { x.Id, x.OperationDateUtc, x.Description, x.ExternalReference, x.Kind, x.Amount, x.Currency, x.ReportedBalance, SystemBalance = running, Difference = x.ReportedBalance.HasValue ? running - x.ReportedBalance.Value : (decimal?)null, x.ReconciliationStatus }; }).Reverse();
