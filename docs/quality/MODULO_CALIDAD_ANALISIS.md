...
| **Estado C2 (07/09/2026):** | **EN CURSO — corte vertical 1.** Snapshot `ProcedureSnapshotJson` + `ExternalDocumentCodesJson` + `InstructionCode` al crear informe; estado `Draft` hasta `POST /reports/{id}/approve` (`RequireTechnicalDirector`); UI listado “Aprobar DT” + “Ver trazabilidad SGC” en impresión. Pendiente: termómetro (`MetrologyInstrument`), PG14-R3/R4 generados, PG09 R2, hoja de vida, vistas Linked IT. | — |
