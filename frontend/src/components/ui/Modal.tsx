...
export function Modal({ open, onClose, title, children, footer, closeOnOverlay = true, contentStyle }: ModalProps) {
  ...
  // Solo al abrir: enfocar el primer campo (no el botón ✕) y armar trap de Tab/Escape.
  // No depende de onClose: si no, cada re-render del padre roba el foco al tipear.
  useEffect(() => {
    if (!open) return;
    ...
    const first = firstField ?? card?.querySelector<HTMLElement>(FOCUSABLE);
    // rAF: el contenido del modal (children) puede montarse en el mismo tick
  ...
  return (
    <div className="modal-backdrop" onClick={closeOnOverlay ? onClose : undefined}>
      <div
        ref={cardRef}
